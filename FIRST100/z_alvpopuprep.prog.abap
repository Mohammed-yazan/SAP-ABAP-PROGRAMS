...
*DECLARATIONS
DATA: ITAB TYPE STANDARD TABLE OF ZSTR01.
...
*
FORM SUBR_OPENSQL .
SELECT BUKRS BSART LIFNR BEDAT EBELN
   INTO TABLE ITAB FROM EKKO WHERE
  LIFNR EQ VENDOR AND BEDAT IN DATE.
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  SUBR_OUTPUT1
*&---------------------------------------------------------------------*
...
ENDFORM.
*INTEACTIVE POPUP WINDOW.
FORM SUBR_EVENT USING PV1 TYPE SY-UCOMM PV2 TYPE SLIS_SELFIELD.
  DATA: VPONO(10) TYPE N.
  IF PV2-FIELDNAME = 'EBELN'.
    VPONO = PV2-VALUE.
     SELECT EBELN AEDAT LGORT INTO TABLE PITAB FROM EKPO WHERE EBELN EQ VPONO.
     IF SY-SUBRC EQ 0.
       PERFORM SUBR_FCAT.
...
CALL FUNCTION 'REUSE_ALV_POPUP_TO_SELECT'
  EXPORTING
  I_TITLE                        = 'PURCHASE ORDER ITEMS INFO '
